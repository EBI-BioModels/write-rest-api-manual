...
def get_new_metadata(biomd_id, old_metadata, master):
    ...
    if "modellingApproach" in old_metadata and old_metadata["modellingApproach"] is not None:
        ...
        metadata["modellingApproach"] = old_metadata["modellingApproach"]["name"]
    ...
    for filename in newfiles:
        if filename in oldfiles:
            ...
            if "sedml" in filename:
                ...
                entry = {"name": filename,
                         "description": "Plotted results from original curation."}
            elif "curation_notes" in filename:
                entry = {"name": filename,
                         "description": "Curator's notes from original curation."}
